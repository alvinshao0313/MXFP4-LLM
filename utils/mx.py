...
def model_with_matmul(model, args=None):
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, LlamaSdpaAttention):
            ...
            logging.info('Quantized forward using torch.nn.functional.scaled_dot_product_attention is not supported. Please set attn_implementation=eager when you load model')
